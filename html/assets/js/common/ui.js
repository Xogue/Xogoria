$(document).ready(function () {
    ...
    $('.uiVeryShortChain, .uiShortChain, .uiMediumChain, .uiLongChain, .uiVeryLongChain').each(function () {
        ...
        const randomLength = Math.floor(Math.random() * (chainLengthMax - chainLengthMin + 1)) + chainLengthMin;
        this.style.setProperty('--chain-length', randomLength + 'px');
    });

    for (let i = 1; i <= anchorCount; i++) {
        const elementA = document.getElementsByClassName('js' + currentPage + 'Anchor' + i + 'A')[0];
        const elementB = document.getElementsByClassName('js' + currentPage + 'Anchor' + i + 'B')[0];
