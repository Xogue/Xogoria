...
class FileMap implements Iterator {
    ...
    public function findFullFilepath(string $filename): ?string {
        return $this->searchTree($this->treeArray, $filename);
    }
    ...
    public function copyTree(string $destination) {
        $this->copyDirectory($this->rootPath, $destination);
    }
    ...
    private function copyDirectory(string $source, string $destination) {
        ...
        foreach ($items as $item) {
            ...
            if (is_dir($srcPath)) {
                $this->copyDirectory($srcPath . DIRECTORY_SEPARATOR, $destPath . DIRECTORY_SEPARATOR);
            } else {
    ...
    public function current(): mixed {
        return $this->treeArray[$this->keys[$this->position]];
    }
