...
class TwitchClipBridge {
    ...
    public function downloadClip( string $clipId, string $destPath ): bool {
        ...
        if ( $downloadUrl === null ) {
            $this->tracker->info(
                "No download URL available from official API. Trying to get it from thumbnail heuristic.",
                __FILE__,
            );
            ...
            if ( $downloadUrl === null ) {
                $this->tracker->error(
                    "Failed to derive download URL from thumbnail for clip ID: " . $clipId,
                    __FILE__,
                );
        ...
        if ( !$response->isOk( ) ) {
            $this->tracker->error(
                "Failed to download clip ID: " . $clipId . ". " . $response->summary( ),
                __FILE__,
            );
    ...
    private function getClipJson( string $url ) {
        ...
        if ( !$response->isOk( ) ) {
            $this->tracker->error(
                "Failed to get clip data from Twitch API. " . $response->summary( ),
                __FILE__,
            );
    ...
    private function getDownloadUrl( string $clipId ): ?string {
        ...
        $row = $jsonData[ "data" ][ 0 ];
        $landscapeUrl = isset( $row[ "landscape_download_url" ] )
            ? (string) $row[ "landscape_download_url" ]
            : "";
        $portraitUrl = isset( $row[ "portrait_download_url" ] )
            ? (string) $row[ "portrait_download_url" ]
            : "";
