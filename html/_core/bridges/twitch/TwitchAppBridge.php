...
class TwitchAppBridge {
    ...
    // MAGIC FUNCTIONS
    public function __construct( TwitchAppContext $twitchAppContext ) { $this->twitchAppContext = $twitchAppContext; }
    ...
    private function checkCode( ): bool|string {
        $code = $_GET[ "code" ] ?? "";
        if ( !$code ) {
            new Logger( Logger::CHANNEL_WEB )->warning( "Twitch callback is missing its code" );
            return false;
        }
        return $code;
    }
    ...
    private function checkState( ): bool {
        ...
        $state = $_GET[ "state" ] ?? "";
        if ( !$expected ) {
            new Logger( Logger::CHANNEL_WEB )->warning( "Twitch auth session state is missing" );
            return false;
        }
        if ( !$state ) {
            new Logger( Logger::CHANNEL_WEB )->warning( "Twitch callback state is missing" );
            return false;
        }
        if ( !hash_equals( $expected, $state ) ) {
            new Logger( Logger::CHANNEL_WEB )->warning( "Twitch callback state is invalid" );
            return false;
        }
        return true;
    }
