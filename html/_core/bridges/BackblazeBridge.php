...
class BackblazeBridge {
    ...
    public function uploadClip(string $clipId, string $filePath): ?string {
        ...
        return rtrim($this->publicBaseUrl, '/') . '/xogue29_clips/' . rawurlencode($clipId) . '.mp4';
    }
    ...
    private function authorize(): bool {
        ...
        if (!$response->isOk()) {
            $this->logger->error('Backblaze authorization failed', ['response' => $response->summary()]);
            return false;
        }
    ...
    private function getClipBucketId(): bool {
        ...
        if (!$response->isOk()) {
            $this->logger->error('Backblaze bucket lookup failed', ['response' => $response->summary()]);
            return false;
        }
    ...
    private function getUploadUrl(): ?array {
        $response = CurlController::postJson($this->privateLoader->getDetail('uploadUrl'), ['bucketId' => $this->clipBucketId])
            ->authorization($this->authorization['authorizationToken'])
            ->send();

        if (!$response->isOk()) {
            $this->logger->error('Backblaze upload URL request failed', ['response' => $response->summary()]);
            return null;
        }
    ...
    private function uploadFile(array $urlResponse, string $filePath, string $clipId): bool {
        ...
        if (!$response->isOk()) {
            $this->logger->error('Backblaze file upload failed', ['response' => $response->summary()]);
            return false;
        }
