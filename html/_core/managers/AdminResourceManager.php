...
final class AdminResourceManager {
    ...
    public function definition(string $resource): array {
        ...
        if (!is_array($definition)) { throw new InvalidArgumentException('Unknown admin resource'); }
        return $definition;
    }
    ...
    public function list(string $resource): array {
        ...
        $sql = 'SELECT ' . implode(', ', $columns) . ' FROM ' . $definition['table'] . " ORDER BY {$primaryKey} ASC";
    ...
    public function save(string $resource, array $input): bool {
        ...
        if ($originalKey === null || $originalKey === '') {
            ...
            if (empty($definition['fields'][$primaryKey]['generated'])) {
                $data[$primaryKey] = $this->normalizeValue($definition['fields'][$primaryKey], $input[$primaryKey] ?? null);
            }
    ...
    private function normalize(array $definition, array $input): array {
        ...
        foreach ($definition['fields'] as $name => $field) {
            if (!empty($field['generated']) && ($input[$name] ?? '') === '') { continue; }
    ...
    private function normalizeValue(array $field, mixed $value): mixed {
        return match ($field['type'] ?? 'text') {
            ...
            'url' => $value === '' ? '' : (filter_var($value, FILTER_VALIDATE_URL) ?: throw new InvalidArgumentException('Invalid URL')),
            default => trim((string) $value),
        };
    }

    private function typeChar(array $field): string {
        return in_array($field['type'] ?? '', ['integer', 'boolean'], true) ? 'i' : 's';
    }
}
