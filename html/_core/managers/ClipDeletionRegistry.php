...
final class ClipDeletionRegistry {
    ...
    public function record(string $clipId, string $twitchUrl, ?string $backblazeUrl, string $requestedBy): array {
        $records = $this->all();
        $record = [
            'clipId' => $clipId,
        ...
        if (!safeWriteJson(self::PATH, $records)) { throw new RuntimeException('Unable to save clip deletion record'); }
        $this->logger->warning('Clip deletion requested', ['clip_id' => $clipId, 'requested_by' => $requestedBy]);
        return $record;
    }
}
