...
class MySqlManager {
    ...
    public function selectUserWithId(string $userId): array {
        $rows = $this->mySqlBridge->select( 'SELECT * FROM users WHERE platformUserId = ? LIMIT 1', 's', [$userId] );
        return is_array($rows) ? ($rows[0] ?? []) : [];
    }

    public function selectUserWithUsername(string $username): array {
        $rows = $this->mySqlBridge->select( 'SELECT * FROM users WHERE username = ? LIMIT 1', 's', [$username] );
        return is_array($rows) ? ($rows[0] ?? []) : [];
    }
    ...
    public function getUserBalance(string $userId): int {
        return $this->selectUserWithId($userId)['gemBalance'] ?? 0;
    }

    public function setUserBalance(string $userId, int $amount): bool {
        return $this->mySqlBridge->update( 'users', ['gemBalance' => $amount], 'platformUserId = ?', 's', [$userId] );
    }
    ...
    public function addToUserBalance(string $userId, int $amount): bool | int {
        ...
        if ($currentBalance + $amount > 1500 && $userId !== '66369644') { // exclude Xogue from limits
        ...
        $success = $this->mySqlBridge->update( 'users', ['gemBalance' => $newBalance], 'platformUserId = ?', 's', [$userId] );
    ...
    public function subtractFromUserBalance(string $userId, int $amount): bool {
        $currentBalance = $this->selectUserWithId($userId)['gemBalance'] ?? 0;
        $newBalance = $currentBalance - $amount;
        return $this->mySqlBridge->update( 'users', ['gemBalance' => $newBalance], 'platformUserId = ?', 's', [$userId] );
    }
    ...
    private function isInt(mixed $value) {
