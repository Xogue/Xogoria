...
final class ClipManager {
    ...
    public function getClipInfoFromIds(array $clipIds): array {
        ...
        return $this->indexRows($this->database->selectFromJson(
    ...
    public function getAllClipInfo(): array {
        return $this->indexRows($this->database->selectFromJson('SELECT * FROM ' . self::TABLE . ' ORDER BY admin_seen_at DESC, clip_id ASC', '', []));
    }

    public function getAllReviewedClipInfo(): array {
        return $this->indexRows($this->database->selectFromJson(
            'SELECT * FROM ' . self::TABLE . ' WHERE review_status = ?', 'i', [1]
        ));
    }

    public function approve(string $clipId, array $twitch): bool {
        $sql = 'INSERT INTO ' . self::TABLE . ' (clip_id, enabled, review_status, admin_seen, admin_seen_at, tw_title, tw_creator, tw_view_count, tw_created_at, tw_thumbnail_url, tw_duration)
            VALUES (?, 1, 1, 1, NOW(), ?, ?, ?, ?, ?, ?)
    ...
    public function setReviewStatus(string $clipId, int $status): bool {
        $sql = 'INSERT INTO ' . self::TABLE . ' (clip_id, enabled, review_status, admin_seen, admin_seen_at)
    ...
    public function increasePlayCount(string $clipId): bool {
        return $this->database->execute('UPDATE ' . self::TABLE . ' SET play_count = play_count + 1 WHERE clip_id = ?', 's', [$clipId]);
    }

    public function setLocalFileUrl(string $clipId, string $url): bool { return $this->update($clipId, ['local_url' => $url, 'has_local_file' => $url !== '' ? 1 : 0]); }
    public function setAudioNormalized(string $clipId, bool $normalized): bool { return $this->update($clipId, ['audio_normalized' => $normalized ? 1 : 0]); }

    private function update(string $clipId, array $fields): bool {
        $success = $this->database->update(self::TABLE, $fields, 'clip_id = ?', 's', [$clipId]);
        if (!$success) { $this->logger->error('Clip metadata update failed', ['clip_id' => $clipId, 'fields' => array_keys($fields)]); }
        return $success;
    }
    ...
    private function indexRows(array $rows): array {
        ...
        foreach ($rows as $row) {
            ...
            if ($id !== '') { $indexed[$id] = $this->normalize($row); }
        }
        return $indexed;
    }
