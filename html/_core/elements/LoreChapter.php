...
class LoreChapter {
    ...
    public function addStream( LoreStream $stream ) { $this->streams[ $stream->getChapterId( ) . ": " . $stream->getDate( ) ] = $stream; }
    public function addAudio( LoreAudio $audio ) { $this->audio[ $audio->getChapterId( ) ] = $audio; }
    ...
    public function assembleChapter( array $chapterTemplatePack ): string {
        ...
        $output = $this->replaceValues(
            $chapterTemplatePack[ "chapter" ],
            $chapterTemplatePack[ "subChapter" ],
        );
