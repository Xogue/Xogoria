...
class Interaction {
    ...
    public function getCurrentCost( ): int    { return $this->cost * $this->duration; }
    public function getPanelHtml( )  : string { return $this->replaceValues( $this->panelTemplate ); }
    ...
    public function getCommandArray( array $details ): array {
        foreach ( $details as $key => $value ) {
            $this->commands = array_combine(
                array_map( function ( $command ) use ( $key, $value ) { return str_replace( $key, $value, $command ); }, array_keys( $this->commands ) ),
