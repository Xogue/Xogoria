...
class WorkerResult {
    ...
    public static function failure(string $message, string $code = 'request_failed', int $httpStatus = 400): self {
    ...
    public function addMeta(string $key, mixed $value): void { $this->meta[$key] = $value; }
    ...
    public function toJson(): string {
        return json_encode($this->toArray(), JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_INVALID_UTF8_SUBSTITUTE) ?: '{"success":false,"code":"json_error"}';
    }
    ...
    // PRIVATE FUNCTIONS
    private function processBool( bool $data ): void {
        $this->success = $data;
    }
    ...
    private function processFloat( float $data ): void {
        $this->success = $data !== 0.0;
    }

    private function processString( string $data ): void {
        $this->success = $data !== '';
    }
    ...
    private function processObject( object $data ): void {
        $this->success = !empty( (array) $data );
    }
