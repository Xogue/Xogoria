...
class CollectionWorker implements WorkerInterface {
    ...
    public function process(): WorkerResult {
        $result = match($this->inputDataContext->getAction()) {
            'name' => $this->mySqlManager->fetchDataByDetail('monster', $this->inputDataContext->getMonster()),
            'objective' => $this->mySqlManager->fetchDataByDetail('objective', $this->inputDataContext->getObjective()),
            'quote' => $this->mySqlManager->fetchDataByDetail('quote', $this->inputDataContext->getQuote()),
            default => false,
        };
