...
final class AdminController {
    ...
    public function requireAdmin(bool $api = false): UserContext {
        ...
        if (!$user->isAdmin()) {
            $this->services->logger($api ? Logger::CHANNEL_API : Logger::CHANNEL_WEB)
                ->warning('Admin access denied', ['user_id' => $user->getUserId()]);
            if ($api) { ApiController::authFailed('Administrator access is required.'); }
    ...
    public function csrfToken(): string {
        if (empty($_SESSION['adminCsrfToken'])) { $_SESSION['adminCsrfToken'] = bin2hex(random_bytes(32)); }
