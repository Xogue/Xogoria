...
try {
    if ($domain === 'resource') {
        ...
        if (!$success) { ApiController::error('The database operation failed.', 500); }
        ApiController::sendJson(['success' => true, 'rows' => $manager->list($resource)]);
    }

    if ($domain === 'config' && $action === 'save') {
        if (!$admin->configs()->save((string) ($input['config'] ?? ''), (string) ($input['source'] ?? ''))) {
    ...
    $services->logger(Logger::CHANNEL_API)->exception($error, ['domain' => $domain, 'action' => $action]);
