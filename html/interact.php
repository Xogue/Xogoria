...
<body>
        ...
        <div class="adminPanel">
            ...
            <div class="gameSelect">
                ...
                <select name="gameSelector">
                    <?php foreach ( $gameManager->getAllGames() as $gameName => $game ):
    ...
    <section class="interactions uiHidden" data-tab="<?php echo $typeName; ?>">
        <div class="cardList">
            <?php
                foreach ( $type->getInteractions() as $key => $interaction ) {
    ...
    <div class="spawnWrap uiHidden">
        <div class="spawnRows">
            <?php
                if ( isset( $profileInteractions["spawn"] ) ) {
    ...
    <section class="intPanel" data-tab="special" hidden>
        <div class="cardList mb-10">
            <?php
                if ( isset( $profileInteractions["special"] ) ) {
                    $batClaim      = $profileInteractions["special"]['batClaim'];
                    $replaceKeys   = ["{COOLDOWN}", "{BLOCKED}", "{LABEL}", "{DESCRIPTION}"];
                    $replaceValues = [$batClaim->getCooldown(), implode( ', ', Sanitize::getBlockedWords() ), $batClaim->getLabel(), $batClaim->getDescription()];
